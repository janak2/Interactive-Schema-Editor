import os, sys, json, datetime, logging, base64, threading, concurrent.futures
...
class GSClient:
    ...
    @classmethod
    def _get_threadpool(cls):
        if cls._threadpool is None:
            cls._threadpool = concurrent.futures.ThreadPoolExecutor(max_workers=cls.max_workers)
        return cls._threadpool

    # use a rate limited threadpool for when i/o is backed up
    # self._threadpool.map(self.get, iter_chunks)
    ...
    def get_chunks(self):
        # Yield file chunks using a class singleton threadpool
